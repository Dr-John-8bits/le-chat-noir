...
test("home can resolve Camembert électrique at the start of the new Saturday evening sequence", async ({ page }) => {
  ...
  await expect(focusTitles.nth(2)).toHaveText("Le Pseudocumentaire de l'espace");
});
...
test("home can resolve When Day Chokes a Radio once the Saturday 19h block starts", async ({ page }) => {
  ...
  await expect(focusTitles.nth(1)).toHaveText("Le Pseudocumentaire de l'espace");
});

test("home can resolve Citron at the start of the new Sunday evening sequence", async ({ page }) => {
  await mockHomeCurrentShow(page, {
    nowIso: "2026-04-19T16:35:00.000Z",
    sinceIso: "2026-04-19T16:03:00.000Z",
    show: "Citron",
    trackTsIso: "2026-04-19T16:20:00.000Z",
  });
  ...
  const focusTitles = page.locator(".today-focus__title");
  await expect(focusTitles).toHaveCount(3);
  await expect(focusTitles.nth(0)).toHaveText("Citron");
  await expect(focusTitles.nth(1)).toHaveText("Messe Noire");
  await expect(focusTitles.nth(2)).toHaveText("Documents de terrain");
});

test("home can resolve Documents de terrain late on Sunday night", async ({ page }) => {
  await mockHomeCurrentShow(page, {
    nowIso: "2026-04-19T20:30:00.000Z",
    sinceIso: "2026-04-19T20:01:00.000Z",
    show: "Documents de terrain",
    trackTsIso: "2026-04-19T20:18:00.000Z",
  });

  await page.goto("/");

  const focusTitles = page.locator(".today-focus__title");
  await expect(focusTitles).toHaveCount(1);
  await expect(focusTitles.nth(0)).toHaveText("Documents de terrain");
});
